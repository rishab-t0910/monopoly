...
Edit on 19/05/21: Sorted out some bugs with the program. Occassionally the program will miss out some runs - solved that! Added the option for the user to query (find) values, as well as some usability stuff (allow the user to re-confirm their run, exit at any point). In the coming days, I'll look to improve the quality of the code, as right now it's a bit clunky and a tad unreadable. 
